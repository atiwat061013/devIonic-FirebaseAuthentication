...
export class SigninPage implements OnInit {
  ...
  db = getDatabase();
  constructor(public formBuilder: FormBuilder, private loaderService: LoaderService, private alertService: AlertService, private router: Router) {
    ...
    if (!this.signinForm.valid) {
      ...
      signInWithEmailAndPassword(this.auth, this.signinForm?.value?.email, this.signinForm?.value?.password)
        ...
        .catch(async (error) => {
          ...
          console.log('[catch] signInWithEmailAndPassword', 'errorCode => ' + errorCode + ' errorMessage => ' + errorMessage);
          // ..
        });
